  ...
  class S3
    ...
    verify_uploadable def upload(uploadable)
      ...
      if uploadable.is_a?(Refile::File) and uploadable.backend.is_a?(S3) and uploadable.backend.access_key_id == access_key_id
        object(id).copy_from(copy_source: [@bucket_name, uploadable.backend.object(uploadable.id).key].join("/"), acl: 'public-read')
      else
        object(id).put(body: uploadable, content_length: uploadable.size, acl: 'public-read')
      end
